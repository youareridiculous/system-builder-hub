"""
End-to-end tests for Blank Canvas template builds
"""
import pytest
import json
import uuid
...
class TestBlankTemplateBuild:
    """Test cases for Blank Canvas template builds"""

    def test_blank_build_creation_via_api(self, client, mock_db_connection):
        """Test creating a Blank Canvas build via API"""
        ...
        with patch('src.builds_api.ensure_builds_table'), \
            ...
            # Blank Canvas build data
            build_data = {
                'name': 'My Blank Project',
                'description': 'Starting from scratch with a clean slate',
                'template': 'blank',
                'use_llm': True,
                'llm_provider': 'openai',
    ...
    def test_blank_build_detail_after_creation(self, client, mock_db_connection):
        """Test getting Blank Canvas build details after creation"""
        mock_db, mock_cursor = mock_db_connection
        
        test_build_id = str(uuid.uuid4())
        
        with patch('src.builds_api.ensure_builds_table'):
            # Mock Blank Canvas build exists
            test_build = {
                'id': test_build_id,
                'name': 'My Blank Project',
                'description': 'Starting from scratch with a clean slate',
            ...
            assert data['name'] == 'My Blank Project'
            assert data['description'] == 'Starting from scratch with a clean slate'
            assert data['template'] == 'blank'
            assert data['status'] == 'initializing'
            assert 'created_at' in data

    def test_blank_build_auto_progression_simulation(self, client, mock_db_connection):
        """Test Blank Canvas build auto-progression in dev mode"""
        mock_db, mock_cursor = mock_db_connection
        ...
        with patch('src.builds_api.ensure_builds_table'):
            ...
            progression_states = [
                {'id': test_build_id, 'name': 'My Blank Project', 'description': 'Starting from scratch with a clean slate', 'template': 'blank', 'status': 'initializing', 'created_at': '2025-08-28 04:52:05.046183'},
                {'id': test_build_id, 'name': 'My Blank Project', 'description': 'Starting from scratch with a clean slate', 'template': 'blank', 'status': 'running', 'created_at': '2025-08-28 04:52:05.046183'},
                {'id': test_build_id, 'name': 'My Blank Project', 'description': 'Starting from scratch with a clean slate', 'template': 'blank', 'status': 'completed', 'created_at': '2025-08-28 04:52:05.046183'}
            ]
    ...
    def test_blank_build_logs_after_completion(self, client, mock_db_connection):
        """Test getting Blank Canvas build logs after completion"""
        mock_db, mock_cursor = mock_db_connection
        ...
        with patch('src.builds_api.ensure_build_logs_table'), \
            ...
            test_logs = [
                ...
                },
                {
                    'id': 'log-2',
                    'build_id': test_build_id,
                    'message': 'Blank Canvas template loaded',
    ...
    def test_tasks_integration_after_blank_build(self, client, mock_db_connection):
        """Test tasks integration after Blank Canvas build completion"""
        mock_db, mock_cursor = mock_db_connection
        ...
        with patch('src.tasks_api.ensure_tasks_schema_and_backfill'):
            ...
            task_data = {
                'title': 'Add custom functionality'
            }
    ...
    def test_blank_build_missing_name_returns_400(self, client, mock_db_connection):
        """Test that Blank Canvas build creation with missing name returns 400"""
        mock_db, mock_cursor = mock_db_connection
        
        with patch('src.builds_api.ensure_builds_table'), \
             patch('src.builds_api.ensure_build_logs_table'):
            
            # Missing name in Blank Canvas build data
            build_data = {
                'description': 'Starting from scratch with a clean slate',
                'template': 'blank',
                'use_llm': True,
    ...
    def test_blank_build_logs_not_found_returns_404(self, client, mock_db_connection):
        """Test that Blank Canvas build logs for non-existent build returns 404"""
        mock_db, mock_cursor = mock_db_connection
    ...
    def test_blank_build_with_missing_template_returns_400(self, client, mock_db_connection):
        """Test that Blank Canvas build with missing template returns 400"""
        mock_db, mock_cursor = mock_db_connection
        
        with patch('src.builds_api.ensure_builds_table'), \
             patch('src.builds_api.ensure_build_logs_table'):
            
            # Missing template in Blank Canvas build data
            build_data = {
                'name': 'My Blank Project',
                'description': 'Starting from scratch with a clean slate',
                'use_llm': True,
    ...
    def test_blank_build_json_consistency(self, client, mock_db_connection):
        """Test that Blank Canvas build objects have consistent JSON structure"""
        mock_db, mock_cursor = mock_db_connection
        
        test_build_id = str(uuid.uuid4())
        
        with patch('src.builds_api.ensure_builds_table'):
            # Mock Blank Canvas build
            test_build = {
                'id': test_build_id,
                'name': 'My Blank Project',
                'description': 'Starting from scratch with a clean slate',
            ...
            for field in required_fields:
                assert field in data, f"Blank Canvas build object missing required field: {field}"
            
            # Verify Blank Canvas-specific values
            assert data['template'] == 'blank'
            assert data['name'] == 'My Blank Project'
