...
function Deals() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Filter deals by account if accountId is provided
      const filteredDeals = accountId 
        ? dealsData.filter(deal => deal.account_id === parseInt(accountId))
        : dealsData;
  ...
  const handleCreate = async () => {
    try {
      const data = { ...formData }
      if (data.account_id === '' || data.account_id === 'none') delete data.account_id
      if (data.contact_id === '' || data.contact_id === 'none') delete data.contact_id
  ...
  const handleEdit = async () => {
    try {
      ...
      if (data.close_date) {
        data.close_date = data.close_date.split('T')[0];
      }
      
      await api.put(`/deals/${selectedDeal.id}`, data)
  ...
  const handleDelete = async () => {
    try {
      await api.del(`/deals/${selectedDeal.id}`)
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            {accountId ? `Deals - ${accounts.find(a => a.id === parseInt(accountId))?.name || 'Account'}` : 'Deals'}
          </h1>
          <p className="mt-2 text-gray-600">
            {accountId ? 'Manage deals for this account' : 'Track your sales opportunities'}
          </p>
        </div>
        ...
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              />
              <p className="text-xs text-gray-500">Click outside the date picker to confirm your selection</p>
            </div>
            <div className="grid gap-2">
