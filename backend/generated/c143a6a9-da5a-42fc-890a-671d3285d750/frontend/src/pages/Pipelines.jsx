...
import { Plus, Edit, Trash2, FileText } from 'lucide-react', Edit, Trash2
...
function Pipelines() {
  ...
  const handleCreate = async () => {
    try {
      await api.post('/api/pipelines/', formData)
  ...
  const handleEdit = async () => {
    try {
      await api.put(`/api/pipelines/${selectedPipeline.id}`, formData)
  ...
  const handleDelete = async () => {
    try {
      await api.del(`/api/pipelines/${selectedPipeline.id}`)
