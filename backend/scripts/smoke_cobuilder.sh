...
echo
echo "== Apply path =="
APPLY_JSON='{"message":"Build Venture OS — Step-1 additive change","apply":true}'
curl -s -X POST http://127.0.0.1:5001/api/cobuilder/ask \
  -H "Content-Type: application/json" -H "X-Tenant-ID: demo" \
  -d "$APPLY_JSON" | jq '.success,.data.applied,.data.apply.file,.data.apply.bytes_written,.data.apply.sha256'

echo
echo "== Inspect written file =="
curl -s "http://127.0.0.1:5001/api/cobuilder/files/inspect?path=src/tenant/model.py" \
  | jq '.ok,.data.exists,.data.size,.data.sha256'
