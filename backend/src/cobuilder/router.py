...
class CoBuilderRouter:
    ...
    def _create_mock_adapter(self):
        ...
        def _mock_call(**kwargs):
            ...
            content = (
                ...
                '"summary":"Added tenant_description field to Tenant model"'
                '}'
            )
    ...
    def _classify_intent(self, message: str) -> str:
        ...
        if any(word in message for word in ["build", "create", "generate", "make", "add", "insert", "modify", "edit", "change"]):
            return "build"
    ...
    def _handle_build(self, message: str, tenant_id: str, dry_run: bool, remaining_time: float = None) -> Dict[str, Any]:
        ...
        # Try NL patcher first for edit requests (behind feature flag)
        if os.getenv('COB_NL_TRANSLATOR_ENABLED', 'true').lower() == 'true':
            try:
                from .nl_patcher import NLPatchTranslator, COB_DRY_RUN, COB_MAX_DIFF_LINES, COB_STRICT_IMPORTS
                
                # Get per-request overrides from message context (if available)
                config = {
                    'dry_run': COB_DRY_RUN,
                    'max_diff_lines': COB_MAX_DIFF_LINES,
                    'strict_imports': COB_STRICT_IMPORTS
                }
                
                translator = NLPatchTranslator(config=config)
                patch = translator.translate(message)
                
                if patch:
                    logger.info(f"NL patcher detected edit request for {patch.target_file}")
                    
                    # Apply the patch with full metadata tracking
                    result = translator.apply_patch(patch, step_id=f"build_{tenant_id}", role="nl_patcher")
                    
                    # Create structured response using ResponseBuilder
                    from .response_builder import create_nl_patch_response
                    return create_nl_patch_response(
                        step_id=f"build_{tenant_id}",
                        tenant_id=tenant_id,
                        patch_result=result,
                        intent="edit_request",
                        message=message
                ...
                logger.warning(f"NL patcher failed: {e}, falling back to generator")
        ...
        if self.generator and not dry_run:
            try:
                ...
                # Build a better prompt for the generator
                user_msg = f"""\
Task:
{message}
...
Constraints:
- Single file only.
- If creating a new file, place it under 'src/'.
- Keep changes minimal and additive where possible.
- Provide a correct unified diff and the full post-change content.
"""
                ...
                return {
                    ...
                    "file": result.file,
                    "diff": result.diff,
                    "content": result.content,  # NEW: full post-change file text
