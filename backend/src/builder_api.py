...
def save_builder_state():
    ...
    try:
        ...
        # Audit the save
        try:
            from .obs.audit import audit_builder_event
            from .tenancy.context import get_current_tenant_id
            tenant_id = get_current_tenant_id()
            audit_builder_event('save', state.project_id, {
...
def generate_build():
    ...
    try:
        ...
        # Audit the build generation
        try:
            from .obs.audit import audit_builder_event
            audit_builder_event("generate", project_id, {
                "mode": "sync" if not async_mode else "async",
                "pages": len(emitted_pages),
                "apis": len(emitted_apis),
                "tables": len(emitted_tables),
        ...
        try:
            ...
            analytics.track(
                ...
                event='builder.generate.completed',
