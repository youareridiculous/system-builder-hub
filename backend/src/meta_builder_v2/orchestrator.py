...
# V3 Integration
from ..meta_builder_v3.types import RetryState, AutoFixOutcome
...
class MetaBuilderOrchestrator:
    ...
    def __init__(self):
        ...
        # Initialize v3 adapter for auto-fix integration
        self.v3_adapter = None
        self.retry_states = {}  # Track retry state per run
    
    def _init_v3_adapter(self):
        """Initialize v3 adapter if not already initialized."""
        if self.v3_adapter is None:
            from ..meta_builder_v3.adapter import V3AutoFixAdapter
            self.v3_adapter = V3AutoFixAdapter(self)
    ...
    async def _handle_step_failure_v3(self, run: BuildRun, step: BuildStep, logs: str, 
                                    artifacts: List[Dict[str, Any]], db_session, context: AgentContext) -> Dict[str, Any]:
        """Handle step failure using v3 auto-fix system."""
        self._init_v3_adapter()
        
        # Initialize retry state for this run if not exists
        if str(run.id) not in self.retry_states:
            # Get settings for this tenant
            settings = feature_flags.get_meta_v3_settings(context.tenant_id, str(run.id), db_session)
            self.retry_states[str(run.id)] = RetryState(
                max_total_attempts=settings.max_total_attempts,
                max_per_step_attempts=settings.max_per_step_attempts
            )
        
        retry_state = self.retry_states[str(run.id)]
        
        # Create logs artifact if not already present
        logs_artifact = create_build_artifact(
            run_id=run.id,
            artifact_type="logs",
            content=logs,
            metadata={"failure": True, "step_id": str(step.id)}
        )
        db_session.add(logs_artifact)
        db_session.commit()
        
        # Classify failure using v3
        signal = self.v3_adapter.classify(step, logs, artifacts)
        
        # Execute auto-fix
        outcome = await self.v3_adapter.auto_fix(context, run, step, signal, retry_state)
        
        # Handle outcome
        if outcome == AutoFixOutcome.RETRIED:
            # Schedule retry with backoff
            backoff_seconds = self.v3_adapter.next_backoff_seconds(signal, retry_state, context.tenant_id)
            
            # Update retry state
            retry_state.per_step_attempts[str(step.id)] = retry_state.per_step_attempts.get(str(step.id), 0) + 1
            retry_state.total_attempts += 1
            retry_state.last_backoff_seconds = backoff_seconds
            ...
            approval_gate = create_approval_gate(
                ...
                status="pending",
                metadata={
                    "failure_signal": signal.__dict__,
                    "step_name": step.name
                }
            )
            ...
            return {
                ...
                "outcome": "escalated",
                "should_retry": False,
                "approval_gate_id": str(approval_gate.id)
            }
    ...
    async def _execute_autofix_step(self, run_context: RunContext, db_session, context: AgentContext) -> Dict[str, Any]:
        ...
        try:
            ...
            # Try v3 auto-fix first, fallback to v2
            try:
                # Initialize v3 adapter
                self._init_v3_adapter()
                
                # Use v3 auto-fix if available
                if self.v3_adapter:
                    # Get failure logs from previous step
                    failure_logs = ""
                    if run_context.reports:
                        last_report = run_context.reports[-1]
                        if "error" in last_report:
                            failure_logs = str(last_report["error"])
                    
                    # Handle with v3
                    v3_result = await self._handle_step_failure_v3(
                        run, step, failure_logs, run_context.artifacts, db_session, context
                    )
                    
                    if v3_result["success"]:
                        step.status = StepStatus.SUCCEEDED
                        step.finished_at = datetime.utcnow()
                        step.output = v3_result
                        db_session.commit()
                        
                        return {
                            "success": True,
                            "fixed": True,
                            "v3_outcome": v3_result.get("outcome")
                        }
                    else:
                        # V3 couldn't fix, continue with v2
                        logger.info("V3 auto-fix failed, falling back to v2")
                        
            except Exception as e:
                logger.warning(f"V3 auto-fix not available, using v2: {e}")
