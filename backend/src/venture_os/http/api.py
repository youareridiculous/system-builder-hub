...
def seed_demo():
    ...
    admin = User(id='seed_admin', tenant_id=tenant_id, email='seed@example.com', name='Seed Admin', roles=[Role.ADMIN])
    created = 0
...
def list_entities():
    ...
    if not tenant_id: 
        return jsonify({'ok': False, 'error': 'missing X-Tenant-ID'}), 400
    try:
