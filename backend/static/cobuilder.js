(function(){
  ...
  async function cobuilderSend(){
    ...
    try {
      ...
      // Show Apply button when we have both file and diff
      if (d.file && d.diff) {
        addApplyButton(d.file, d.diff, inflight.sentText);
      }
  ...
  function addApplyButton(file, diff, originalPrompt) {
    const container = document.createElement('div');
    container.className = 'cb-apply-container';
    
    const applyBtn = document.createElement('button');
    applyBtn.textContent = 'Apply change';
    applyBtn.className = 'cb-apply';
    applyBtn.onclick = async () => {
      applyBtn.disabled = true;
      applyBtn.textContent = 'Applying...';
      
      try {
        const r = await fetch('/api/cobuilder/ask', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-Tenant-ID': tenantId
        ...
        if (j?.success && j?.data?.applied) {
          addMessage(
            `✅ Applied to ${j.data.apply.file} (${j.data.apply.bytes_written} bytes)\nsha256: ${j.data.apply.sha256}`,
            false
          );
        } else {
          addMessage('Apply failed — check logs.', false);
        }
      } catch(e) {
        addMessage('Apply failed — network error.', false);
      } finally {
