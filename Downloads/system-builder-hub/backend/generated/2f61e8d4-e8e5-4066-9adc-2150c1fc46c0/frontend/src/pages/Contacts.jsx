...
function Contacts() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Filter contacts by account if accountId is provided
      const filteredContacts = accountId 
        ? contactsData.filter(contact => contact.account_id === parseInt(accountId))
        : contactsData;
  ...
  const handleDelete = async () => {
    try {
      await api.del(`/contacts/${selectedContact.id}`)
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            {accountId ? `Contacts - ${accounts.find(a => a.id === parseInt(accountId))?.name || 'Account'}` : 'Contacts'}
          </h1>
          <p className="mt-2 text-gray-600">
            {accountId ? 'Manage contacts for this account' : 'Manage your contact information'}
          </p>
        </div>
