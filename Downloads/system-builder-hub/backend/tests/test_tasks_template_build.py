"""
End-to-end tests for Task Manager template builds
"""
import pytest
import json
import uuid
...
class TestTasksTemplateBuild:
    """Test cases for Task Manager template builds"""

    def test_tasks_build_creation_via_api(self, client, mock_db_connection):
        """Test creating a Task Manager build via API"""
    ...
    def test_tasks_build_detail_after_creation(self, client, mock_db_connection):
        """Test getting Task Manager build details after creation"""
        mock_db, mock_cursor = mock_db_connection
        
        test_build_id = str(uuid.uuid4())
        
        with patch('src.builds_api.ensure_builds_table'):
            # Mock Task Manager build exists
            test_build = {
                'id': test_build_id,
                'name': 'My Task Manager',
                'description': 'Simple task tracking and project management',
            ...
            assert data['name'] == 'My Task Manager'
            assert data['description'] == 'Simple task tracking and project management'
            assert data['template'] == 'tasks'
            assert data['status'] == 'initializing'
            assert 'created_at' in data

    def test_tasks_build_auto_progression_simulation(self, client, mock_db_connection):
        """Test Task Manager build auto-progression in dev mode"""
        mock_db, mock_cursor = mock_db_connection
        ...
        with patch('src.builds_api.ensure_builds_table'):
            ...
            progression_states = [
                {'id': test_build_id, 'name': 'My Task Manager', 'description': 'Simple task tracking and project management', 'template': 'tasks', 'status': 'initializing', 'created_at': '2025-08-28 04:52:05.046183'},
                {'id': test_build_id, 'name': 'My Task Manager', 'description': 'Simple task tracking and project management', 'template': 'tasks', 'status': 'running', 'created_at': '2025-08-28 04:52:05.046183'},
                {'id': test_build_id, 'name': 'My Task Manager', 'description': 'Simple task tracking and project management', 'template': 'tasks', 'status': 'completed', 'created_at': '2025-08-28 04:52:05.046183'}
            ]
    ...
    def test_tasks_build_logs_after_completion(self, client, mock_db_connection):
        """Test getting Task Manager build logs after completion"""
        mock_db, mock_cursor = mock_db_connection
        ...
        with patch('src.builds_api.ensure_build_logs_table'), \
            ...
            test_logs = [
                ...
                },
                {
                    'id': 'log-2',
                    'build_id': test_build_id,
                    'message': 'Task Manager template loaded',
    ...
    def test_tasks_integration_after_tasks_build(self, client, mock_db_connection):
        """Test tasks integration after Task Manager build completion"""
        mock_db, mock_cursor = mock_db_connection
    ...
    def test_tasks_build_missing_name_returns_400(self, client, mock_db_connection):
        """Test that Task Manager build creation with missing name returns 400"""
        mock_db, mock_cursor = mock_db_connection
        
        with patch('src.builds_api.ensure_builds_table'), \
             patch('src.builds_api.ensure_build_logs_table'):
            
            # Missing name in Task Manager build data
            build_data = {
                'description': 'Simple task tracking and project management',
                'template': 'tasks',
                'use_llm': True,
    ...
    def test_tasks_build_logs_not_found_returns_404(self, client, mock_db_connection):
        """Test that Task Manager build logs for non-existent build returns 404"""
        mock_db, mock_cursor = mock_db_connection
    ...
    def test_tasks_build_with_missing_template_returns_400(self, client, mock_db_connection):
        """Test that Task Manager build with missing template returns 400"""
        mock_db, mock_cursor = mock_db_connection
        
        with patch('src.builds_api.ensure_builds_table'), \
             patch('src.builds_api.ensure_build_logs_table'):
            
            # Missing template in Task Manager build data
            build_data = {
                'name': 'My Task Manager',
                'description': 'Simple task tracking and project management',
                'use_llm': True,
    ...
    def test_tasks_build_json_consistency(self, client, mock_db_connection):
        """Test that Task Manager build objects have consistent JSON structure"""
        mock_db, mock_cursor = mock_db_connection
        
        test_build_id = str(uuid.uuid4())
        
        with patch('src.builds_api.ensure_builds_table'):
            # Mock Task Manager build
            test_build = {
                'id': test_build_id,
                'name': 'My Task Manager',
                'description': 'Simple task tracking and project management',
            ...
            for field in required_fields:
                assert field in data, f"Task Manager build object missing required field: {field}"
            
            # Verify Task Manager-specific values
            assert data['template'] == 'tasks'
            assert data['name'] == 'My Task Manager'
            assert 'task tracking' in data['description']
